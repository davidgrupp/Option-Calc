defmodule OptionCalc.Services.OptionDataCache do
  ...
  def get_expirations(pid, symbol) do
    get_value(pid, :expirations, symbol, fn symbol -> @option_repo.read_expirations(symbol) |> Task.await end)
  end

  def get_expirations_async(pid, symbol) do
    get_value_async(pid, :expirations, symbol, fn symbol -> @option_repo.read_expirations(symbol) |> Task.await end)
  end

  def get_strike_prices(pid, symbol, date) do
    get_value(pid, :strikes, {symbol, date}, fn {symbol, date} -> @option_repo.read_strike_prices(symbol, date) |> Task.await end)
  end
  ...
  def get_value(pid, section, key, query) do
    Agent.get_and_update(pid, fn cache -> 
      if cached = cache[section][key] do
        { cached.value, cache }
      else
        result = query.(key)
        cached_value = %{ value: result, cached_on: DateTime.utc_now, status: :ready }
        { result, Map.put(cache, section, Map.put(cache[section], key, cached_value )) }
      end
    end)
  end
